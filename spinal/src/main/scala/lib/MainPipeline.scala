  ...
  def build(): Unit ={
    ...
    for((key,info) <- inputOutputKeys) {
      ...
      for (stageIndex <- info.insertStageId to info.lastOutputStageId;
        ...
        stage.output(key)
        //println(s"${key.toString()} -> ${info.lastOutputStageId +1}")
      ...
      for (stageIndex <- info.insertStageId to info.lastInputStageId) {
        ...
        stage.input(key)
        println(s"${key.toString()}: ${stageIndex}")
        ...
        if (inputDefault != null) {
          if (stageIndex == info.insertStageId) {
            ...
            inputDefault := RegNextWhen(stageBefore.output(key), stage.dontSample.getOrElse(key, Nil).foldLeft(!stage.arbitration.isStuck)(_ && !_)).setName(s"${stageBefore.getName()}_to_${stage.getName()}_${key.getName()}_r")
          }
        }
      }
    }
